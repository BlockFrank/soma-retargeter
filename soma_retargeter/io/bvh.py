...
class BVHImporter(object):
    ...
    def bvh_parser(cls, file_path, remove_namespace=True):
        ...
        if not os.path.exists(file_path) or os.path.splitext(file_path)[-1].lower() != '.bvh':
            raise ValueError(f'Invalid BVH file path: {file_path}')
        ...
        for i, line in enumerate(data):
            ...
            if token[0] == 'ROOT':
                if len(token) < 2:
                    raise ValueError(f'Malformed BVH file: ROOT keyword has no name at line {i + 1}')
                joint_name = token[1]
                if remove_namespace and ':' in joint_name:
                    joint_name = joint_name.split(':')[-1]
                joint = BVHJoint(joint_name)
                joint.path = f'/{joint_name}'
                jointWalker.append(joint)
                joints.append(joint)
            # Joint
            elif token[0] == 'JOINT':
                if len(token) < 2:
                    raise ValueError(f'Malformed BVH file: JOINT keyword has no name at line {i + 1}')
                if not jointWalker:
                    raise ValueError(f'Malformed BVH file: JOINT "{token[1]}" appears before any ROOT at line {i + 1}')
                joint_name = token[1]
                if remove_namespace and ':' in joint_name:
                    joint_name = joint_name.split(':')[-1]
                joint = BVHJoint(joint_name)
                joint.parent = jointWalker[-1]
                joint.path = f'{joint.parent.path}/{joint_name}'
                jointWalker.append(joint)
                joints.append(joint)
            # End Site
            ...
            # }
                ...
                if ignoreNextBrackets:
                    ...
                    if not jointWalker:
                        raise ValueError(f'Malformed BVH file: unbalanced closing brace at line {i + 1}')
                    jointWalker.pop(-1)
            elif token[0] == 'MOTION':
                motionLine = i
                break
        ...
        if not joints:
            raise ValueError(f'Malformed BVH file: no ROOT joint found in HIERARCHY section of {file_path}')
        return joints[0]
    ...
    def get_frame_range(bvh_joint_obj):
        ...
        if bvh_joint_obj.frame_time <= 0:
            raise ValueError(f"Invalid frame time: {bvh_joint_obj.frame_time}. Must be greater than zero.")
